...
{
    public partial class FormGenerarSolicitudCotizacion : Form
    {
        public FormGenerarSolicitudCotizacion()
        {
            InitializeComponent();
        }
        ...
        private void GenerarSolicitudCotizacion_Load(object sender, EventArgs e)
        {
            MostrarNroSolCot();
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            if (txtPrefUnit.Text != "")
            {
                if (Convert.ToInt32(txtUnidad.Text) != 0)
                {
                    ...
                    MessageBox.Show("serie insertada");
                    Mostrar_Cantidad();
                    Listar_series_nroSol();
                }
                else
                {
                    MessageBox.Show("Ingresar serie");
                }
            }
            else
            {
        ...
        private void InsertarSeriesPrecintos()
        {
            if (txtPrefRango.Text != "" && txtPrefPrcnto.Text != "")
            {
                 if (Convert.ToInt32(txtDesde.Text) != 0 && Convert.ToInt32(txtDesde.Text) != 0)
                    ...
                    {
                        MessageBox.Show("Ingresar rango de las series y precintos");
                    }
            }
            else
            {
